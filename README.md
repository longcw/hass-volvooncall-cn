...
由于油量传感器本身不准，可以把每笔记录的升数改成加油站的实际数值（例如 40.82 升）。在车辆卡片的「实测油耗」一行点开即可修改日期、里程、升数，也可以删除记录或手动补记（补记时把日期和里程改成当时的数值即可）。同样的操作也可以直接调用服务：

| 服务 | 说明 |
| --- | --- |
| `volvooncall_cn.log_refuel` | 补记一次加油，参数 `vin`、`liters`，可选 `odometer`、`at` |
| `volvooncall_cn.update_refuel` | 修正某笔记录，参数 `vin`、`record_id`，可选 `liters`、`odometer`、`at` |
| `volvooncall_cn.delete_refuel` | 删除某笔记录，参数 `vin`、`record_id` |

> 记录按日期排序，因此修改日期也会改变「这一箱油对应哪段里程」。
